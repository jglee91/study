...
router.post('/workspaces', isLoggedIn, async (req, res, next) => {
  ...
  try {
    ...
    const workspace = await Workspace.create(
      { name: req.body.workspace, url: req.body.url, OwnerId: req.user.id },
      { transaction: t }
    );
    await workspace.addMembers(req.user.id, { transaction: t });
    const channel = await Channel.create(
      { name: '일반', WorkspaceId: workspace.id },
      { transaction: t }
    );
...
router.post(
  ...
  async (req, res, next) => {
    ...
    try {
      ...
      const channel = await Channel.create(
        { name: req.body.name, WorkspaceId: workspace.id },
        { transaction: t }
      );
      await channel.addMembers(req.user.id, { transaction: t });
      await t.commit();
...
router.post(
  '/workspace/:workspace/dms/:id/chats',
  isLoggedIn,
  async (req, res, next) => {
    try {
      const workspace = await Workspace.findOne({
...
router.post(
  ...
  async (req, res, next) => {
    ...
    try {
      const workspace = await Workspace.findOne({
        where: { url: req.params.workspace },
        include: [{ model: Channel, where: { name: '일반' } }],
      });
      if (!workspace) {
        await t.rollback();
        return res.status(404).send('존재하지 않는 워크스페이스입니다.');
...
router.delete(
  ...
  async (req, res, next) => {
    try {
      ...
      const channel = workspace.Channesl.find(
        (v) => v.name === decodeURIComponent(req.params.channel)
      );
      if (!channel) {
        return res.status(404).send('존재하지 않는 채널입니다.');
...
router.get('/workspaces/:workspace/users/:id', async (req, res, next) => {
  try {
    const workspcae = await Workspace.findOne({
      where: { url: req.params.workspace },
    });
    if (!workspace) {
      return res.status(404).send('존재하지 않는 워크스페이스입니다.');
...
router.post('/users', isNotLoggedIn, async (req, res, next) => {
  try {
    ...
    await channel.addMemebers(user);
